...
response=$(curl -s -X POST "http://127.0.0.1:8000/api/first-question/" \
...
response2=$(curl -s -X POST "http://127.0.0.1:8000/api/next-question/" \
-H "Content-Type: application/json" \
-d '{
  "memberId": "'"$memberId"'",
...
feedback_response=$(curl -s -X POST "http://127.0.0.1:8000/api/feedback/" \
-H "Content-Type: application/json" \
-d '{
  "memberId": "'"$memberId"'",
  "chatRoomId": "'"$chatRoomId"'",
